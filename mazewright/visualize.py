...
def render(
    ...
    """
    fig = Figure(figsize=(maze.cols * cell_size, maze.rows * cell_size))
    FigureCanvasAgg(fig)
    ax = fig.subplots()
    ...
    # Top border
    segments.append(
        [(0, maze.rows * cell_size), (maze.cols * cell_size, maze.rows * cell_size)]
    )
    # Bottom border
    ...
    # Right border
    segments.append(
        [(maze.cols * cell_size, 0), (maze.cols * cell_size, maze.rows * cell_size)]
    )
    ...
    ax.plot(
        start_x, start_y, "o", color="green", markersize=cell_size * 12, label="Start"
    )
    ...
    ax.plot(
        finish_x, finish_y, "s", color="red", markersize=cell_size * 12, label="Finish"
    )
...
def save_svg(
    ...
    if solution_path and len(solution_path) > 1:
        ...
        svg_content.write(
            f'  <polyline points="{points_str}" fill="none" stroke="{solution_color}" '
            f'stroke-width="{wall_width * 1.5}" opacity="0.8"/>\n'
        )

    # Draw walls
    svg_content.write(
        f'  <g stroke="{wall_color}" stroke-width="{wall_width}" stroke-linecap="square">\n'
    )
    ...
    svg_content.write(
        f'    <line x1="{width}" y1="0" x2="{width}" y2="{height}"/>\n'
    )  # Right
    ...
    for row in range(maze.rows):
        for col in range(maze.cols):
            ...
            if row > 0 and cell.has_wall(Wall.NORTH):
                svg_content.write(
                    f'    <line x1="{x_left}" y1="{y_top}" x2="{x_right}" y2="{y_top}"/>\n'
                )
            ...
            if col > 0 and cell.has_wall(Wall.WEST):
                svg_content.write(
                    f'    <line x1="{x_left}" y1="{y_top}" x2="{x_left}" y2="{y_bottom}"/>\n'
                )

            if col < maze.cols - 1 and cell.has_wall(Wall.EAST):
                svg_content.write(
                    f'    <line x1="{x_right}" y1="{y_top}" x2="{x_right}" y2="{y_bottom}"/>\n'
                )
    ...
    svg_content.write(
        f'  <circle cx="{start_x}" cy="{start_y}" r="{cell_size * 0.3}" '
    ...
    svg_content.write(
        f'  <rect x="{finish_x - cell_size * 0.3}" y="{finish_y - cell_size * 0.3}" '
